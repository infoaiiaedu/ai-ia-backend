...
def get_services_status():
    ...
    try:
        result = subprocess.run(
            ["pgrep", "-x", "nginx"], capture_output=True, timeout=5
        )
        services["nginx"] = {
...
def get_uptime_stats():
    ...
    health_checks = HealthCheck.objects.filter(
        checked_at__gte=yesterday, service="django"
    )
    ...
    avg_response_time = (
        health_checks.filter(response_time_ms__isnull=False).aggregate(
    ...
    return {
        ...
        "last_check": health_checks.first().checked_at
