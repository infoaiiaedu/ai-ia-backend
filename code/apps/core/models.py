...
class Subject(models.Model):
    ...
    price = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="ფასი"
    )
    icon = models.JSONField(null=True, blank=True, editable=True, verbose_name="აიქონი")
...
class Topic(models.Model):
    ...
    video = models.JSONField(null=True, blank=True, editable=True, verbose_name="ვიდეო")
    description = models.TextField(blank=True, null=True, verbose_name="აღწერა")

    created_at = models.DateTimeField(
        default=timezone.now, verbose_name="შექმნის თარიღი"
    )
...
class QuizImport(models.Model):
    ...
    name = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="სახელი"
    )
    ...
    processed_at = models.DateTimeField(
        blank=True, null=True, verbose_name="დამუშავების დრო"
    )
    created_at = models.DateTimeField(
        default=timezone.now, verbose_name="შექმნის თარიღი"
    )
...
class Quiz(models.Model):
    ...
    topics = models.ManyToManyField(
        Topic, related_name="quizzes", blank=True, verbose_name="თემები"
    )
    grade = models.ForeignKey(
        Grade,
        on_delete=models.CASCADE,
...
class Question(models.Model):
    ...
    quiz = models.ForeignKey(
        Quiz, related_name="questions", on_delete=models.CASCADE, verbose_name="ქვიზი"
    )
    text = models.TextField(verbose_name="კითხვის ტექსტი")
    question_type = models.CharField(
        max_length=10, choices=QUESTION_TYPES, verbose_name="კითხვის ტიპი"
    )
    level = models.PositiveSmallIntegerField(default=1, verbose_name="დონე")
    ...
    correct_text_answer = models.TextField(
        blank=True, null=True, verbose_name="სწორი პასუხი (ღია კითხვებისთვის)"
    )
...
class Answer(models.Model):
    ...
    is_correct = models.BooleanField(
        default=False, verbose_name="სწორია?"
    )  # only used for MCQs
    order = models.PositiveSmallIntegerField(default=0, verbose_name="მიმდევრობა")
